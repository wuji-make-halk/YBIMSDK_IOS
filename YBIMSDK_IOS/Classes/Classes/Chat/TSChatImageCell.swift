...
class TSChatImageCell: TSChatBaseCell {
    ...
    override func setCellContent(_ model: ChatModel) {
        ...
        if let localThumbnailImage = model.localThumbnailImage {
            ...
            self.chatImageView.ts_setImageWithURLString(model.thumbURL)
        }
        self.setNeedsLayout()
    }
    ...
    override func layoutSubviews() {
        ...
        if (model.imageWidth != nil) {
            imageOriginalWidth = CGFloat(Float(model.imageWidth!)!)
        }
        
        if (model.imageHeight != nil) {
            imageOriginalHeight = CGFloat(Float(model.imageHeight!)!)
        }
    ...
    class func layoutHeight(_ model: ChatModel) -> CGFloat {
        ...
        let imageOriginalWidth = CGFloat(Float(model.imageWidth ?? "100")!)
        let imageOriginalHeight = CGFloat(Float(model.imageHeight ?? "800")!)
