...
class TSChatSystemCell: UITableViewCell {
    ...
    func setCellContent(_ model: ChatModel) {
        self.model = model
        self.infomationLabel.text = model.payload
    }
    ...
    override func layoutSubviews() {
        ...
        self.infomationLabel.ts_setFrameWithString(model.payload ?? "", width: kChatInfoLabelMaxWdith)
    ...
    class func layoutHeight(_ model: ChatModel) -> CGFloat {
        ...
        let stringHeight: CGFloat = (model.payload ?? " ").ts_heightWithConstrainedWidth(kChatInfoLabelMaxWdith, font: kChatInfoFont)
        height += stringHeight + kChatInfoLabelPaddingTop*2
        model.cellHeight = height + 20
        return model.cellHeight
    }
