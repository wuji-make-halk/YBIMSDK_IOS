...
class TSChatTextParser: NSObject {
    ...
    fileprivate class func enumerateEmotionParser(_ attributedText: NSMutableAttributedString, fontSize: CGFloat) {
        ...
        for emotion: NSTextCheckingResult in emoticonResults {
            ...
            //QQ 表情的文件名称
            let imageString =  (Bundle(for: YBChatViewController.self).path(forResource: theImageName + "@2x", ofType: "png"))! 
            let emojiText = NSMutableAttributedString.yy_attachmentString(withEmojiImage: UIImage(contentsOfFile: imageString)!, fontSize: fontSize + 1)
