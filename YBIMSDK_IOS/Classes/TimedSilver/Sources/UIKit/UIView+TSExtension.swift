...
public extension UIView {
    ...
    class func ts_Nib() -> UINib {
        let hasNib: Bool = Bundle(for: self).path(forResource: self.ts_className, ofType: "nib") != nil
        guard hasNib else {
            assert(!hasNib, "Nib is not exist")
            return UINib()
        }
        return UINib(nibName: self.ts_className, bundle:Bundle(for: self)
        )
    }
    ...
    class func ts_viewFromNib<T>(_ aClass: T.Type) -> T {
        ...
        } else if (Bundle(for: T.self as! AnyClass) != nil) {
            let bundel = Bundle(for: T.self as! AnyClass)
             return UINib(nibName: name, bundle:bundel).instantiate(withOwner: nil, options: nil)[0] as! T
        } else {
